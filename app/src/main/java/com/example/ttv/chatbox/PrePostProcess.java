...
class ResultDetect {
    ...
    Float score;
    Rect rect;
    String objectName; // Thêm trường mới cho tên vật thể


    public ResultDetect(int cls, Float output, Rect rect, String objectName) {
        this.classIndex = cls;
        this.score = output;
        this.rect = rect;
...
public class PrePostProcess {
     ...
     */
    static ArrayList<ResultDetect> nonMaxSuppression(ArrayList<ResultDetect> boxes, int limit, float threshold) {

        // Do an argsort on the confidence scores, from high to low.
        Collections.sort(boxes,
                new Comparator<ResultDetect>() {
                    @Override
                    public int compare(ResultDetect o1, ResultDetect o2) {
                        return o1.score.compareTo(o2.score);
                    }
                });

        ArrayList<ResultDetect> selected = new ArrayList<>();
        ...
        for (int i=0; i<boxes.size() && !done; i++) {
            if (active[i]) {
                ResultDetect boxA = boxes.get(i);
                selected.add(boxA);
                if (selected.size() >= limit) break;

                for (int j=i+1; j<boxes.size(); j++) {
                    if (active[j]) {
                        ResultDetect boxB = boxes.get(j);
    ...
    static ArrayList<ResultDetect> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ArrayList<ResultDetect> results = new ArrayList<>();
        for (int i = 0; i< mOutputRow; i++) {
            if (outputs[i* mOutputColumn +4] > mThreshold) {
                ...
                String objectName = (mClasses != null && cls < mClasses.length) ? mClasses[cls] : "Unknown"; // Lấy tên của vật thể từ danh sách tên lớp

                Rect rect = new Rect((int)(startX+ivScaleX*left), (int)(startY+top*ivScaleY), (int)(startX+ivScaleX*right), (int)(startY+ivScaleY*bottom));
                ResultDetect result = new ResultDetect(cls, outputs[i*mOutputColumn+4], rect, objectName);
                results.add(result);
            }
        }
